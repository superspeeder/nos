...
static inline uint64_t bsf64(uint64_t n) {
    uint64_t out = 0;
    asm volatile("bsf %0, %1" : "=r"(out) : "r"(n));
    return out;
}
...
static void *_pmem_alloc_page_from_rmap() {
    if (_regionmap_page == nullptr) return nullptr;
...
static void _pmem_free_page_region(struct regionmap_entry* region, void* ptr) {
    ...
    if ((old_mask & mask) == 0) ++(region->available_count);
}
...
void pmem_create_usable_region(uint64_t paddr, uint64_t size) {
    ...
    for (uint64_t i = 0 ; i < aligned_size ; i += RMAP_ENTRY_MAX_PAGE_COUNT * 0x1000) {
        ...
        // TODO: write function for allocating multiple contiguous pages both for high and lowmem areas (the only reason we even distinguish between the two is because lowmem being standardized means we can use it's available space to bootstrap space for region bitmaps from the memory map.
